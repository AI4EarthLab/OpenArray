...
namespace oa {
  namespace funcs {
    ...
    template <typename T>
    ArrayPtr consts(MPI_Comm comm, const Shape& s, T val, int stencil_width = 1, DeviceType dev_type = CPU_AND_GPU) {
      ...
      ArrayPtr ap = ArrayPool::global()->get(comm, s, stencil_width, data_type, dev_type);
      ...
      #else 
        oa::gpu::set_buffer_consts((T*)ap->get_buffer(), size, val);
      #endif
      return ap;
    }
    ...
    ArrayPtr consts(MPI_Comm comm, const vector<int>& x, const vector<int>& y, 
      const vector<int>&z, T val, int stencil_width = 1,  DeviceType dev_type = CPU_AND_GPU) {
      int data_type = oa::utils::to_type<T>();
      ...
      ArrayPtr ap = ArrayPool::global()->get(comm, x, y, z, stencil_width, data_type, dev_type);
      Box box = ap->get_local_box();
      int size = box.size_with_stencil(stencil_width);
      #ifndef __HAVE_CUDA__
        oa::internal::set_buffer_consts((T*)ap->get_buffer(), size, val);
    ...
    ArrayPtr seqs(MPI_Comm comm, const Shape& s, 
      int stencil_width = 1, int data_type = DATA_INT, DeviceType dev_type=CPU_AND_GPU); 

    ArrayPtr seqs(MPI_Comm comm, const vector<int> &x, const vector<int> &y, 
      const vector<int> &z, int stencil_width = 1, int data_type = DATA_INT, DeviceType dev_type=CPU_AND_GPU);

    // according to partiton pp, transfer src to A
    // A = transfer(src, pp)
    ArrayPtr transfer(const ArrayPtr &src, const PartitionPtr &pp, DeviceType dev_type=CPU_AND_GPU);
...
/*
    ...
    void local_sub(const ArrayPtr &ap, int x, int y, int z, T* val)  {
      ...
      case DATA_DOUBLE:
       ...
       switch(ap->get_data_type()) {
      case DATA_INT:
        *val = oa::gpu::get_buffer_local_sub((int*)ap->get_buffer(), 
            local_box, x, y, z, sw);
        break;
      case DATA_FLOAT:
        *val = oa::gpu::get_buffer_local_sub((float*)ap->get_buffer(), 
            local_box, x, y, z, sw);
        break;
      case DATA_DOUBLE:
        *val = oa::gpu::get_buffer_local_sub((double*)ap->get_buffer(), 
            local_box, x, y, z, sw);
        break;
      }
      #endif
...
*/
    ...
    void set_local(const ArrayPtr &ap, int x, int y, int z, T val) {
      ...
      switch(ap->get_data_type()) {
      case DATA_INT:
        oa::gpu::set_buffer_local((int*)ap->get_buffer(), 
            local_box, x, y, z, (int)val, sw);
        break;
      case DATA_FLOAT:
        oa::gpu::set_buffer_local((float*)ap->get_buffer(), 
            local_box, x, y, z, (float)val, sw);
        break;
      case DATA_DOUBLE:
        oa::gpu::set_buffer_local((double*)ap->get_buffer(), 
            local_box, x, y, z, (double)val, sw);
        break;
      }
      #endif
    ...
    void update_ghost(ArrayPtr ap, DeviceType dev_type = CPU);
    ...
    void update_ghost_start(ArrayPtr ap, vector<MPI_Request> &reqs, int direction = -1, 
        oa_int3 lb = {{0,0,0}}, oa_int3 rb = {{0,0,0}}, DeviceType dev_type = CPU_AND_GPU); 
    ...
    void calc_outside(ArrayPtr &ap, ArrayPtr &B, oa_int3 lbound, oa_int3 rbound);
    ...
    bool is_equal(const ArrayPtr& A, T B){
      ...
      return *A_buf == B;
      #else
      return oa::gpu::is_equal_arrayptr_and_scalar(A, B);
      #endif
    }
    ...
    bool is_equal(const ArrayPtr& A, T* B){
      ...
      return true;
      #else 
      return oa::gpu::is_equal_arrayptr_and_array(A, B);
      #endif
    }
    ...
    ArrayPtr get_seq_array(T* val, const Shape& s){
      ...
      #else
      oa::gpu::copy_buffer((T*)a->get_buffer(), val, size);
      #endif
      return a;
    }
    ...
    void set_ref_const(ArrayPtr& A, const Box& A_box, T val) {
      ...
      if (A->get_data_type() == DATA_INT) {
        #ifndef __HAVE_CUDA__
        oa::internal::set_buffer_subarray_const<int, T>(
          (int*) A->get_buffer(),
          val,box,sub_box,
          pp->get_stencil_width()
        );
        #else
        oa::gpu::set_buffer_subarray_const<int, T>(
          (int*) A->get_buffer(),
          val, box, sub_box,
          pp->get_stencil_width()
        );
      ...
      if (A->get_data_type() == DATA_FLOAT) {
        #ifndef __HAVE_CUDA__
        oa::internal::set_buffer_subarray_const<float, T>(
          (float*) A->get_buffer(),
          val,box,sub_box,
          pp->get_stencil_width()
        );
        #else
        oa::gpu::set_buffer_subarray_const<float, T>(
          (float*) A->get_buffer(),
          val, box, sub_box,
          pp->get_stencil_width()
        );
      ...
      if (A->get_data_type() == DATA_DOUBLE) {
        #ifndef __HAVE_CUDA__
        oa::internal::set_buffer_subarray_const<double, T>(
          (double*) A->get_buffer(),
          val,box,sub_box,
          pp->get_stencil_width()
        );
        #else
        oa::gpu::set_buffer_subarray_const<double, T>(
          (double*) A->get_buffer(),
          val, box, sub_box,
          pp->get_stencil_width()
        );
    ...
    ArrayPtr create_local_array(const Shape& gs, T* buf){
      ...
      #else
       return oa::gpu::set_local_array(gs, buf);
      #endif
    }
    ...
    void set(ArrayPtr& A, const Box& ref_box,
      ...
      #else
      oa::gpu::set_gpu(A, ref_box, buf, buf_shape);
      #endif
    }
