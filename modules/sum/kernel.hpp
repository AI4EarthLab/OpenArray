  ...
  namespace kernel {
    ...
    template <typename T>
      ArrayPtr t_kernel_sum_scalar(vector<ArrayPtr> &ops_ap) {
        ...
        T *all_sum = &temp2;
#ifdef __HAVE_CUDA__
        oa::internal::gpu::buffer_sum_scalar_const(
            (T*) local_sum, 
            (T*) u->get_buffer(),
            u->get_local_box(),
            sw,
...
#ifdef __HAVE_CUDA__
        //ap->memcopy_cpu_to_gpu();// do not need to copy, because get_seq_scalar make the data in gpu
#endif
        return ap;
      }
...
#else
        T * buffer;
        cudaMalloc((void**)&buffer, sizeof(T)*buffersize);
#endif
        ...
        for(int j = 0; j < sp[1]; j++)
        {
          ...
          for(int i = 0; i < sp[0]; i++)
            for(int k = 0; k < sp[2]; k++){
              ...
              if(rankID == sendid){
#ifdef __HAVE_CUDA__
                oa::internal::gpu::buffer_csum_y_const(
                    (T*) ap->get_buffer(),
                    (T*) u->get_buffer(),
                    u->get_local_box(),
                    sw,
        ...
        for(int k = 0; k < sp[2]; k++)
        {
          ...
          for(int i = 0; i < sp[0]; i++)
            for(int j = 0; j < sp[1]; j++){
              ...
              if(rankID == sendid){
#ifdef __HAVE_CUDA__
                oa::internal::gpu::buffer_csum_z_const(
                    (T*) ap->get_buffer(),
                    (T*) u->get_buffer(),
                    u->get_local_box(),
                    sw,
