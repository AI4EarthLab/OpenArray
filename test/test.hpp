...
/*
 ...
 #else
  oa::gpu::set_ghost_consts((int*)ap->get_buffer(), ap->local_shape(), 0, 1);
#endif
*/
  int rk = ap->rank();
...
#else
  oa::utils::print_data(ap->get_cpu_buffer(), sp, DATA_INT);
#endif
*/
  MPI_ORDER_END
}
...
void test_set() {
  // A
  //ArrayPtr ap = oa::funcs::seqs(MPI_COMM_WORLD, {6, 6}, {6, 6, 6}, {1}, 1);
  ArrayPtr ap = oa::funcs::seqs(MPI_COMM_WORLD,  {18, 18, 18}, {1}, 0);
  ap->display("======A======");
  // sub1
  Box box1(4, 6, 5, 12, 0, 0);
