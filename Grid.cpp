...
void Grid::init_grid(char type,
  ...
  case 'C':
    ...
    for (int i = 0; i < 8; i++) {
      #ifdef __HAVE_CUDA__
      x_d[i]->memcopy_gpu_to_cpu();
      #endif 
      vector<MPI_Request> reqs;
      oa::funcs::update_ghost_start(x_d[i], reqs, 3, {{0,0,0}}, {{0,0,0}}, CPU);
      oa::funcs::update_ghost_end(reqs);
      #ifdef __HAVE_CUDA__
      x_d[i]->memcopy_cpu_to_gpu();
      #endif 
      reqs.clear();

      #ifdef __HAVE_CUDA__
      y_d[i]->memcopy_gpu_to_cpu();
      #endif 
      oa::funcs::update_ghost_start(y_d[i], reqs, 3, {{0,0,0}}, {{0,0,0}}, CPU);
      oa::funcs::update_ghost_end(reqs);
      #ifdef __HAVE_CUDA__
      y_d[i]->memcopy_cpu_to_gpu();
      #endif 
      reqs.clear();

      #ifdef __HAVE_CUDA__
      z_d[i]->memcopy_gpu_to_cpu();
      #endif 
      oa::funcs::update_ghost_start(z_d[i], reqs, 3, {{0,0,0}}, {{0,0,0}}, CPU);
      oa::funcs::update_ghost_end(reqs);
      #ifdef __HAVE_CUDA__
      z_d[i]->memcopy_cpu_to_gpu();
      #endif 
      reqs.clear();
    }
