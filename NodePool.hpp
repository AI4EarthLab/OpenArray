...
public:
  ...
  NodePtr get_local_1d(T* val) {
    ...
    #else
    T* d_val;
    CUDA_CHECK(cudaMalloc((void **)&d_val, sizeof(int)*size));
    CUDA_CHECK(cudaMemcpy(d_val, val, sizeof(int) * size, cudaMemcpyHostToDevice));
    oa::gpu::copy_buffer((T*)ap->get_buffer(), (T*)d_val, size);
