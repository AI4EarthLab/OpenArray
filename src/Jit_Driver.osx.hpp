  ...
  private:
    ...
    int insert_gcc(size_t hash, const stringstream& code){
     ...
     pathname<<buffer<<"/kernel_folder";
      ...
      //cmd<<"gcc -shared -fPIC -nostartfiles -xHost -O3 -Ofast -finline -inline-level=2 -finline-functions -no-inline-factor -g -w -o "<<objname.str().c_str()<<" "<<filename.str().c_str();
      cmd<<"gcc -shared -fPIC -O3  -g -w -o "<<objname.str().c_str()<<" "<<filename.str().c_str();
      //cmd<<"pwd";
