...
class Array {
  ...
  DeviceType m_newest_buffer;
  void* m_buffer_gpu;

  public:
  // Constructor, default data type is DATA_DOUBLE

  Array(const PartitionPtr &ptr, int data_type = DATA_DOUBLE, DeviceType dev_type = CPU_AND_GPU);
  ...
  void* get_buffer(DeviceType dt = GPU);
  ...
  void update_lb_ghost_updated(oa_int3 lb);
  void update_rb_ghost_updated(oa_int3 rb);
  bool get_lb_ghost_updated(int dimension);
  bool get_rb_ghost_updated(int dimension);
  void reset_ghost_updated();

  #ifdef __HAVE_CUDA__
  void memcopy_cpu_to_gpu();
  void memcopy_gpu_to_cpu();
  void update_hash();
  void set_newest_buffer(DeviceType dev_type);
  #endif
  void* get_cpu_buffer();
  void* get_gpu_buffer();
...
size_t gen_array_hash(size_t par_hash, int data_type, DeviceType dev_type=CPU);
