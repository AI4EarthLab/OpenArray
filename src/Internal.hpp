...
/*
        for (int k = zs; k < ze; k++) {
          for (int j = ys; j < ye; j++) {
            for (int i = xs; i < xe; i++) {
              sub_buffer[(k-zs)*N*M+j*M+i] = buffer[k * M * N + j * M + i];
            }
          }
        }
*/
        //memcpy(sub_buffer,buffer+zs*M*N,(ze-zs)*M*N*sizeof(T));
        memcpy(sub_buffer+sw*M*N,buffer+zs*M*N+sw*M*N,(ze-zs-sw)*M*N*sizeof(T));
      }
    }
